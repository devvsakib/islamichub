...
export default function Onboarding({ onComplete }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white dark:bg-[var(--color-dark-bg)] flex flex-col">
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none">
        <IslamicPattern opacity={1} color="currentColor" />
      </div>
          ...
          {[1, 2, 3].map(s => (
            <div 
              ...
              className={`h-1.5 rounded-full transition-all duration-500 ${
                s === step ? 'w-8 bg-[var(--color-primary)]' : 
                  ...
                  <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                    {Object.keys(CALCULATION_METHODS).map(m => (
                      <button
                        ...
                        className={`w-full px-4 py-3.5 rounded-2xl text-left text-xs font-bold transition-all border ${
                          data.method === m 
